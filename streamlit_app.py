...
st.write("Enter symptoms separated by commas to predict possible diseases along with their probabilities.")

symptoms_input = st.text_input("Enter Symptoms", "itching, skin rash, nodal skin eruptions")

if st.button('Predict'):
    symptoms_list = [s.strip() for s in symptoms_input.split(',')]
    predicted_diseases = predict_possible_diseases(symptoms_list, top_n=5)

    if predicted_diseases:
        st.write("### Predicted Diseases and Probabilities")
        for disease, probability in predicted_diseases:
            st.write(f"- **{disease}**: {probability}")
    else:
        st.write("No diseases could be predicted. Please check the symptoms")
